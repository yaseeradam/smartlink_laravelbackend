...
import 'merchant_service_request_detail_screen.dart';

class MerchantServiceRequestsScreen extends StatefulWidget {
  const MerchantServiceRequestsScreen({super.key});

  @override
  State<MerchantServiceRequestsScreen> createState() => _MerchantServiceRequestsScreenState();
}

class _MerchantServiceRequestsScreenState extends State<MerchantServiceRequestsScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await ApiClient.instance.getJson('merchant/service-requests');
  ...
  Widget build(BuildContext context) {
    ...
    if (role != AppRole.merchant) {
      return const _NotAuthorizedScreen(title: 'Service requests');
    }
      ...
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            if (!_loading && _error == null && _items.isEmpty) _EmptyCard(isDark: isDark, message: 'No service requests.'),
            if (!_loading && _error == null)
              ..._items.map((r) {
                final id = r['id'];
                final status = (r['status'] as String?) ?? '';
                final requestText = (r['request_text'] as String?) ?? '';
                    ...
                    child: ListTile(
                      onTap: id == null
                          ...
                          : () => Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (_) => MerchantServiceRequestDetailScreen(requestId: id),
                                ),
                              ),
                      leading: Container(
                        width: 44,
                        ...
                        child: const Icon(Icons.home_repair_service_outlined, color: AppTheme.primaryColor),
                      ),
                      title: Text(
                        id == null ? 'Service request' : 'Request #$id',
                        style: Theme.of(context).textTheme.labelLarge?.copyWith(fontWeight: FontWeight.w900),
                      ),
                      subtitle: Text(
                        [
                          if (requestText.trim().isNotEmpty) requestText.trim(),
