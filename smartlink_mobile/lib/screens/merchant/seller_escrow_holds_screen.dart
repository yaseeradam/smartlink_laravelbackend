...
class SellerEscrowHoldsScreen extends StatefulWidget {
  const SellerEscrowHoldsScreen({super.key});

  @override
  State<SellerEscrowHoldsScreen> createState() => _SellerEscrowHoldsScreenState();
}

class _SellerEscrowHoldsScreenState extends State<SellerEscrowHoldsScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final res = await ApiClient.instance.getJson('seller/escrow/holds');
  ...
  Widget build(BuildContext context) {
    ...
    if (role != AppRole.merchant) {
      return const _NotAuthorizedScreen(title: 'Escrow holds');
    }
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Escrow holds')),
      body: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            if (!_loading && _error == null && _items.isEmpty) _EmptyCard(isDark: isDark, message: 'No escrow holds.'),
            if (!_loading && _error == null)
              ..._items.map((h) {
                final id = h['id'];
                final amount = (h['amount'] as String?) ?? '0';
                final status = (h['status'] as String?) ?? '';
                final orderId = h['order_id'];
...
class _EmptyCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.lock_outline, color: isDark ? Colors.white54 : Colors.black45),
          const SizedBox(width: 10),
          Expanded(
            child: Text(
              message,
